# Author: SilentNightSound#7430 , AIRDE
...
# 此版本为GI脚本面向Naraka做特定改造，以便学习其中技术
...
def main():
    ...
    input_attributes = parser.parse_args()

    # 测试过程，必须指定一些属性，由于不触发控制台，所以这里手动输入参数
    input_attributes.name = "dress"
    input_attributes.framedump = r"C:\Users\Administrator\Desktop\NBLoaderV1.1\FrameAnalysis-2023-03-07-085821"
    input_attributes.vb = "1ee262a3"
    input_attributes.vs = "e8425f64cfb887cd"

    character = input_attributes.name.replace(" ", "")
    ...
    if input_attributes.framedump:
        frame_dump_folder = input_attributes.framedump
    else:
    ...
    point_vbs = collect_pointlist_candidates(frame_dump_folder, input_attributes.vs)
    ...
    relevant_ids, first_vss = collect_relevant_ids(frame_dump_folder, input_attributes.vb)
    ...
    model_data, position_vbs, texcoord_vbs = collect_model_data(frame_dump_folder, relevant_ids, input_attributes.force)
    ...
    for i in range(len(model_data)):
        ...
        hash_data.append({})
        if input_attributes.component_names:
            hash_data[i]["component_name"] = input_attributes.component_names[i].replace(" ", "")
        elif i>0:
        ...
        texture_only = False
        if input_attributes.vb[i][0].lower() == "x":
        ...
        # Some objects do not have a separate vb1, and all the data is contained in vb0
            ...
            if input_attributes.has_blend and not point_vb_candidates:
                print("ERORR: Unable to find matching blend coordinates. Please specify the draw IDs to use with --force")
                return

            if not input_attributes.no_blend and not point_vb_candidates:
                print("WARNING: Unable to find vbs to use for position information.")
                return

            if input_attributes.no_blend or (not input_attributes.force_object and not point_vb_candidates):
                ...
                if input_attributes.force_object:
                    correct_vb_id = input_attributes.force_object
                else:
                ...
                hash_data[i]["root_vs"] = input_attributes.vs
            ...
            if input_attributes.remove_sanity:
        ...
        output_results(frame_dump_folder, character, input_attributes.component_names, model_data, vb_merged, position_vb, i, texture_only, base_classification, input_attributes.has_normalmap)
        ...
        for index in object_indexes:
            ...
            for j, texture in enumerate(model_data[i][index][1:]):
                ...
                if texture_hash not in seen_hashes:
                    ...
                    extension = ".dds"
                    if j==0:
                        if input_attributes.has_normalmap:
                    ...
                    elif j==2 and input_attributes.has_normalmap:
                        texture_type = "LightMap"
                    else:
...
def collect_pointlist_candidates(frame_dump_folder, root_vs_hash="e8425f64cfb887cd"):
    ...
    # print(frame_dump_files)
    # 这里会列出frame analyse文件夹中所有的文件，然后在下面一个一个的遍历

    for filename in frame_dump_files:
        draw_id = filename.split("-")[0]
        # 获取开头的索引，这个索引的专业叫法是draw_id，暂且这么认为  000001,000002,...
    ...
    for draw_vb_hash in draw_vb_hashes:
        ...
        for filename in frame_dump_files:
            if draw_vb_hash in filename:
                ...
                if not first_vs and int(draw_id) > 33:
                ...
                if draw_id not in relevant_id_group and (use_lower or int(draw_id)>33):
